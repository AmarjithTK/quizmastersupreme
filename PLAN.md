...
| 2026-09-14 | **Batch generate workflow (§12.6).** Inputs now include **Target** and **Sources** (persisted on the job, placed in the prompt as authoritative context; `PROMPT_VERSION` v3). One generation = one batch view: all candidates shown together with duplicate/possible-duplicate flags (layer-1 exact auto-flagged, everything fuzzy presented §13.6), reject one by one (rejected → never committed), survivors stay together as the approved set, and a single commit action adds the WHOLE set to an existing Q Set or a newly created one (`POST /api/admin/generation-jobs/:id/commit`; job records `committed_set_id`). Migration 0004. **Live verified:** topic+target+sources → 3 clean candidates → reject Q1 → commit to new set → set holds exactly the 2 approved questions. Test count 286 → 290. |\n| 2026-09-14 | **D1 parameter-ceiling bugfix.** `ai_candidates` grew 17 → 26 columns but the ingest chunker still used the old 17, so a 10-candidate job inserted 5-row chunks = 110 bound params > D1's ~100 limit and step 2/2 failed with a wrapped `Failed query`. Chunk size now derives from the table's actual column count (`Object.keys(table).length`) in both the pipeline and the backup restore (widest table 33 cols → 4-row chunks were also over). Regression test: a 10-candidate stub job ingests cleanly. Test count 282 → 286. |\n| 2026-09-14 | **OpenRouter provider routing (only/order).** Migration 0003 adds `provider_only`/`provider_order` to `ai_generation_jobs`; settings screen gains the two comma-separated slug fields; jobs persist routing and the provider body sends exactly `provider` (only/order) — nothing else (no sorting/latency/pricing/ZDR). Model default deepseek/deepseek-v4-flash-0731. Live-verified: settings → job → wire (`providerOnly:["together","deepinfra"], providerOrder:[…]`). Test count 274 → 282. |\n| 2026-09-14 | **Admin AI settings + logging.** New `/admin/settings` (provider locked to OpenRouter, default model editable with presets incl. `deepseek/deepseek-v4-flash-0731`); job creation falls back to the stored model, then `DEFAULT_GENERATION_MODEL` (now deepseek); full `[qms]` log trail across the AI pipeline, provider, storage and routes (masked secrets); error responses log; jobs show error codes; live job ran to succeeded with every stage visible. Test count 270 → 274. |\n| 2026-09-14 | **M14 implemented and verified locally.** Hardening: in-memory rate limiter wired into auth/attempts/answers/admin-step/search (`RATE_LIMITED` 429 with per-bucket windows); root error boundary, global-error and 404 pages; **backup/restore rehearsed twice** — the integration test wipes the DB and proves FTS still searches, and a real `pnpm backup` + `pnpm restore --file` round-trip on the dev DB returned identical row counts (8/14/20/80/31/2/35/2/16/41/15/6/0/36/0/0/0/0) with `questions_fts` rebuilt to 20 rows; `wrangler.staging.jsonc` (isolated Worker + D1, `APP_ENV=staging`); accessibility pass on the quiz runner (skip link, live verdict region, keyboard focus to Next) plus README runbook. **Honest limitations:** no Cloudflare account, so staging deploy, remote restore rehearsal and Lighthouse are unverified. Test count 262 → 270. |\n| 2026-09-14 | **M13 implemented and verified locally.** Coverage-aware generation: `buildCoverageDigest` (exact-topic pass → conjunctive FTS top-up → concept keys → token budget), digest persisted on every job, `preview-digest` admin endpoint, and prompt-acceptance reporting by `prompt_version` surfaced on `/admin/generate`. **Live check:** "Operating Systems" digest is 5 on-topic concepts with 0 off-topic strays — the earlier OR-based FTS pull (Jupiter, binary numbers) is gone; unknown topic → empty block; "Linux kernel" → 2 on-topic. **Honest limitation:** the exit-test half that compares duplicate rates with vs without a digest requires a real model call and stays unverified until an OpenRouter key is configured. Test count 241 → 262. |
